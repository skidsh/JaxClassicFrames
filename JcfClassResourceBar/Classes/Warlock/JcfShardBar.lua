...
function JcfWarlockPowerBar:OnLoad()
...
function JcfWarlockPowerBar:OnEvent(event, ...)
	if event == "PLAYER_REGEN_ENABLED" or event == "PLAYER_REGEN_DISABLED" then
        -- Regen being enabled/disabled is one of the few ways UI is notified that player's combat state has changed
		self:UpdatePower();
    end

	JcfClassResourceBarMixin.OnEvent(self, event, ...);
end

function JcfWarlockPowerBar:UpdatePower()
...
function JcfWarlockPowerBar:UnitPower(unit)
...
JcfWarlockShardMixin = {};

JcfWarlockShardMixin.IncrementSettings = {
...
function JcfWarlockShardMixin:Setup()
	self.fillAmount = nil;
	self:ResetVisuals();
	self:Show();
end

function JcfWarlockShardMixin.OnRelease(framePool, self)
	self:ResetVisuals();
	FramePool_HideAndClearAnchors(framePool, self);
end

function JcfWarlockShardMixin:Update(powerAmount, isBarFull)
	...
	elseif fillAmount >= 1 then -- Full
		...
		local incrementSettings = JcfWarlockShardMixin.IncrementSettings[incrementIndex];
...
function JcfWarlockShardMixin:UpdateFullPowerVisuals(isBarFull)
...
function JcfWarlockShardMixin:ResetVisuals()
	self.emptyToFullAnim:Stop();
	self.fillDoneAnim:Stop();
	self.readyLoopAnim:Stop();
