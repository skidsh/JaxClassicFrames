...
function JcfRuneFrameMixin:OnLoad()
...
function JcfRuneFrameMixin:OnEvent(event, ...)
...
function JcfRuneFrameMixin:UpdateRunes(isSpecChange)
	...
	for newLayoutIndex, runeIndex in ipairs(self.runeIndices) do
		...
		if numNewlyDepletedRunes > 0 and runeButton.visualState ~= JcfRuneButtonMixin.VisualState.Ready then
...
function JcfRuneFrameMixin:RuneButtonComparison(runeAIndex, runeBIndex)
	local runeAButton = self.Runes[runeAIndex];
	local runeBButton = self.Runes[runeBIndex];

	return JcfRuneButtonMixin.CompareRuneButtons(runeAButton, runeBButton);
end

function JcfRuneFrameMixin:OnEnter()
...
function JcfRuneFrameMixin:OnLeave()
	GameTooltip_Hide();
end
...
JcfRuneButtonMixin.VisualState = {
...
function JcfRuneButtonMixin:OnLoad()
...
function JcfRuneButtonMixin:UpdateSpec(specIndex)
...
function JcfRuneButtonMixin:UpdateState()
	...
	if not runeReady then
		...
		elseif previousState ~= JcfRuneButtonMixin.VisualState.Empty then
...
function JcfRuneButtonMixin:PlayDepleteVisuals()
	self.DepleteVisuals.DepleteAnim:Restart();
end

function JcfRuneButtonMixin:UpdateLayoutIndex(layoutIndex)
...
function JcfRuneButtonMixin:ShowAsReady(previousState)
	...
	if not self.CooldownEndingAnim:IsPlaying() and previousState ~= JcfRuneButtonMixin.VisualState.Ready then
		self:SkipToFinalAnimState(self.CooldownEndingAnim);
	end

	self.visualState = JcfRuneButtonMixin.VisualState.Ready;
	self.cooldownEndingStartTime = nil;
	self.Cooldown:Clear();
end

function JcfRuneButtonMixin:ShowAsEmpty()
	if self.CooldownFillAnim:IsPlaying() then
		self:SkipToFinalAnimState(self.CooldownFillAnim);
	end
	...
	self.Cooldown:Clear();
	self.cooldownEndingStartTime = nil;
	self.visualState = JcfRuneButtonMixin.VisualState.Empty;
	self.isNewlyDepleted = true;
end

function JcfRuneButtonMixin:ShowAsOnCooldown(start, duration, previousState)
	...
	if (previousState == nil) or (previousState == JcfRuneButtonMixin.VisualState.Ready) or (previousState == JcfRuneButtonMixin.VisualState.CooldownEnding and isBeforeCooldownEndStartTime) then
	...
	if isBeforeCooldownEndStartTime and timeNowFloored >= math.floor(start) then
		...
		self.visualState = JcfRuneButtonMixin.VisualState.OnCooldown;
	end
end

function JcfRuneButtonMixin:OnCooldownUpdate()
	...
	if timeNow >= self.cooldownEndingStartTime then
		...
		self.cooldownEndingStartTime = nil;
		self.visualState = JcfRuneButtonMixin.VisualState.CooldownEnding;
	end
end

function JcfRuneButtonMixin:SkipToFinalAnimState(animation)
	if animation then
...
function JcfRuneButtonMixin.CompareRuneButtons(runeAButton, runeBButton)
	...
	if runeAState == JcfRuneButtonMixin.VisualState.Ready then
