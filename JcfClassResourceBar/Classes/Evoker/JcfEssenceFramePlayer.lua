...
JcfEssencePowerBar = {};
function JcfEssencePowerBar:UpdatePower()
...
function JcfEssencePowerBar:UpdateChargedPowerPoints()
	self:UpdatePower();
end

function JcfEssencePowerBar:SetupEvoker()
	local showBar = false;
...
JcfEssencePointButtonMixin = { }; 
function JcfEssencePointButtonMixin:OnUpdate(elapsed) 
	local peace,interrupted = GetPowerRegenForPowerType(Enum.PowerType.Essence)
	if (peace == nil or peace == 0) then
		peace = 0.2;
	end
...
function JcfEssencePointButtonMixin:AnimIn(animationSpeedMultiplier, animationElapsedPortion)
	self.EssenceFilling.FillingAnim:SetAnimationSpeedMultiplier(animationSpeedMultiplier);
	self.EssenceFilling.CircleAnim:SetAnimationSpeedMultiplier(animationSpeedMultiplier);
...
function JcfEssencePointButtonMixin:AnimOut()
...
function JcfEssencePointButtonMixin:SetEssennceFull()
	self.EssenceFilling.FillingAnim:Stop();
	self.EssenceFilling.CircleAnim:Stop();
