...
function JcfClassPowerBar:OnLoad()
...
function JcfClassPowerBar:GetUnit()
	local unit = self.unit or self:GetParent():GetParent().unit;
	return unit or "player";
end

function JcfClassPowerBar:SetTooltip(tooltipTitle, tooltip)
...
function JcfClassPowerBar:SetPowerTokens(...)
	local tokens = {}
...
function JcfClassPowerBar:UsesPowerToken(tokenType)
	return self.powerTokens and self.powerTokens[tokenType];
end

function JcfClassPowerBar:OnEvent(event, ...)
...
function JcfClassPowerBar:OnEnter()
...
function JcfClassPowerBar:OnLeave()
	GameTooltip:Hide();
end

function JcfClassPowerBar:Setup()
	local _, class = UnitClass("player");
	local spec = GetSpecialization();
	local showBar = false;
	if ( class == self.class ) then
		if ( not self.spec or spec == self.spec ) then
			JcfPlayerFrame.classPowerBar = self;
...
function JcfClassPowerBar:TurnOn(frame, texture, toAlpha)
...
function JcfClassPowerBar:TurnOff(frame, texture, toAlpha)
	local alphaValue = texture:GetAlpha();
	frame.Fadein:Stop();
	frame.Fadeout:Stop();
	texture:SetAlpha(alphaValue);
